  ...
  <nav>
    ...
    <DropdownMenu>
      ...
      <div class="profile-menu__options">
        <div v-if="auth.status.value === 'authenticated'" class="no-select">
          Logged in as {{ auth.user.value?.name }}
        </div>
        <button
        ...
        <NuxtLink to="/upload" v-if="auth.status.value === 'authenticated'"
          ><Icon name="material-symbols:upload-file-rounded" size="1.5em" />
          <div>Upload</div></NuxtLink
        >
        <NuxtLink to="/profile" v-if="auth.status.value === 'authenticated'">
